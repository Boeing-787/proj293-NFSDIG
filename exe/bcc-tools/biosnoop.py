...
parser = argparse.ArgumentParser(
    ...
    epilog=examples)
parser.add_argument("-Q", "--queue", action="store_true", default=True,
    help="include OS queued time")
parser.add_argument("-d", "--disk", type=str,
    help="trace this disk only")
parser.add_argument("-P", "--pattern", action="store_true", default=True,
    help="display block I/O pattern (sequential or random)")
parser.add_argument("--ebpf", action="store_true",
    help=argparse.SUPPRESS)
parser.add_argument("--output", type=str, default="./traceOutput/biosnoop.csv",
    help="output file")
...
# header
f.write("%-8s," % "TIME")
#f.write("%-14s %-7s %-9s %-1s %-10s %-7s" % ("COMM,", "PID,", "DISK,", "T,", "SECTOR,", "BYTES,"))
f.write("%s,%s,%s,%s,%s,%s," % ("COMM", "PID", "DISK", "T", "SECTOR", "BYTES"))
if args.pattern:
    f.write("%-1s, " % ("P"))
if args.queue:
    f.write("%7s " % ("QUE(ms),"))
...
def print_event(cpu, data, size):
    ...
    if args.pattern:
        ...
        f.write("%-1s," % pattern)
    if args.queue:
        f.write("%7.2f," % (float(event.qdelta) / 1000000))
