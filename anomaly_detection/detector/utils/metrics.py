...
def sliding_anomaly_predict(score: np.ndarray, window_size: int = 70, stride: int = 70, ratio: float = 3):
    '''
    滑动窗口的动态阈值异常预测
    ---
    ### Parameters:
        score: np.ndarray 异常评分
        window_size: int (default = 70) 滑动窗口大小
        stride: int (default = 70) 滑动窗口步长
        ratio: float (default = 3) 标准差比例，例：threshold = mean + `ratio` * std
    ### Return:
